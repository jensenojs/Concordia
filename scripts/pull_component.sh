...
ldd "$OUTPUT/lib/libnvcuda.so" >"$WORK/backend-ldd.txt"
nm -D --defined-only "$OUTPUT/lib/libnvcuda.so" >"$WORK/backend-exports.txt"
for symbol in cuInit cuLaunchKernel cuMemcpyDtoHAsync cuModuleGetGlobal_v2; do
    grep -Eq " [A-Z] ${symbol}$" "$WORK/backend-exports.txt"
done
