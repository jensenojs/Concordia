...
Cargo.lock SHA256固定为`25af8d8ea6826c1e7a0a98dbe28823cdfa6a3bb1fd83cea0fea9b7aaa94d475b`，与本地成功build和旧控制仓冻结副本逐字节一致。`llvm_zluda`通过path dependency读取`ext/llvm-project/llvm-sys`；build只从gitlink`72de6de8...`稀疏恢复该目录，LLVM二进制和链接输入仍来自固定系统LLVM21。`ext/cuda-tile`及完整LLVM源码不参与该profile。
